...
- **App shell** — desktop/management shell for non-terrain routes **as implemented today** (onboarding, pending onboarding, select-establishment, no-establishment, invitations, auth pages). Configuration opérationnelle lives at `/e/{establishmentId}/operational-config` (TerrainShell, desktop web `lg` only; mobile/native redirect to `/e/{id}/reporting`).
- **Spore Platform V1 (planned, not implemented):** a distinct desktop-Web-only shell (`isDesktopWeb`), not TerrainShell and not the current `/onboarding` cream shell. Native and non-desktop Web must not expose it. See [`edb_plateforme_interne_spore_v1-3.md`](../cadrage/edb_plateforme_interne_spore_v1-3.md).
