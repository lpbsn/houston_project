...
Onboarding: Platform `OnboardingDraft` → `complete_onboarding_session_core`. Historical migrations still create then drop `OnboardingProposal` (`0004` / `0024` / `0036`) so empty databases can `migrate` from zero.
