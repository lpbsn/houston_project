...
- Authorization is layered and must not be collapsed:
  ...
  - **Surface permissions** — HTTP entry for a given context. Workspace product/team APIs use `HasActiveMembership` (session gate, not object rules). Path-scoped establishment/organization admin is decided **once** per request (`decide_active_establishment_admin_access` / `decide_organization_admin_access`) and does not substitute for that session gate. Modes: [`identity_membership_domain.md`](identity_membership_domain.md).
