...
- Management shell (`AppShell`) for non-terrain routes: `/organization`, `/organization/establishments/{id}`, onboarding.
- Organization admin (Owners): `/organization` — establishments, members, owners.
- Establishment admin (Owners org-wide + Directors on path): `/organization/establishments/{id}` — overview metrics + memberships; desktop entry to operational config (`/e/{id}/operational-config`, desktop web lg only).
