...
- Observation compose is process-memory (`/reporting` text + local `File` photos; task-observation text). Photos upload only at Envoyer. Envoyer is disabled while offline. **Capacitor Lot 10 done** (régime A). Device QA (nav interne, background sans kill, avion, picker) is manual. Survival after process kill is not in that lot.
