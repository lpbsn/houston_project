...
export function DirectionSection() {
  return (
    <section className="landing-grid-bg relative overflow-hidden bg-spore-forest px-5 py-20 sm:px-8 sm:py-28 lg:px-8 xl:px-12 xl:py-28">
      <div
        aria-hidden="true"
      ...
      <div className="relative mx-auto max-w-5xl lg:max-w-6xl xl:max-w-7xl">
        <Reveal className="mx-auto max-w-2xl text-center lg:max-w-3xl">
          ...
          <h2 className="text-balance text-[clamp(1.6rem,4vw,2.6rem)] font-semibold leading-tight text-white lg:text-[clamp(2.5rem,3vw,3.5rem)]">
            {directionContent.titleBefore}{' '}
            <span className="text-spore-neon">{directionContent.titleAccent}</span>
          </h2>
          <p className="mt-5 text-pretty text-[15px] leading-relaxed text-white/80 sm:text-base lg:text-lg">
            {directionContent.body}
          </p>
        </Reveal>

        <div className="mt-12 grid gap-4 sm:mt-14 sm:grid-cols-3 sm:gap-5 lg:gap-6">
          {directionContent.cards.map((card, index) => (
            <Reveal key={card.number} delay={index * 0.06}>
              <article className="h-full rounded-2xl bg-white p-5 shadow-lg sm:p-6 lg:p-8">
                <span className="inline-flex h-8 w-8 items-center justify-center rounded-lg bg-spore-forest text-xs font-bold text-white">
                  {card.number}
                </span>
                <h3 className="mt-4 text-lg font-semibold text-spore-ink lg:text-xl">{card.title}</h3>
