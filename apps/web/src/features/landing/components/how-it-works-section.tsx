...
export function HowItWorksSection() {
  return (
    <section className="relative overflow-hidden bg-spore-cream px-5 py-16 sm:px-8 sm:py-24">
      <div
        aria-hidden="true"
      ...
      <div className="relative mx-auto max-w-3xl">
        <Reveal>
          <h2 className="mx-auto max-w-2xl text-center text-balance text-[clamp(1.5rem,3.5vw,2.25rem)] font-semibold leading-tight text-spore-forest">
          ...
          {howItWorksContent.steps.map((step, index) => {
            ...
            return (
              <Reveal key={step.number} delay={index * 0.05}>
                <li className="relative grid grid-cols-[auto_1fr] items-start gap-4 sm:gap-6">
                  ...
                  <article className="rounded-2xl bg-white p-5 shadow-[0_12px_40px_rgba(16,59,42,0.08)] ring-1 ring-black/5 sm:p-6">
                    <h3 className="text-lg font-semibold text-spore-forest sm:text-xl">
                      {step.title}
                    </h3>
                    <p className="mt-2 text-[15px] leading-relaxed text-spore-moss">{step.body}</p>
                  </article>
                </li>
              </Reveal>
            )
        ...
        <Reveal className="mt-16 grid gap-10 sm:mt-20 sm:grid-cols-2 sm:gap-8">
          {howItWorksContent.metrics.map((metric) => (
            <div key={metric.value} className="text-center">
              <p className="text-[clamp(3rem,10vw,4.5rem)] font-semibold leading-none tracking-tight text-spore-neon drop-shadow-[0_0_24px_rgba(82,255,154,0.35)]">
                {metric.value}
              </p>
              <p className="mt-3 text-sm text-spore-moss sm:text-base">{metric.label}</p>
            </div>
          ))}
        </Reveal>
      </div>
