...
function PinnedDesktopRow({
  ...
  return (
    <article className="flex flex-col rounded-xl border border-[#E8E6DF] bg-[#F0EFE9] px-3 py-2.5">
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex shrink-0 items-center gap-0.5">
          <time
            className={cn('whitespace-nowrap text-[12px] leading-snug', terrain.textSecondary)}
      ...
      <button
        type="button"
        className="w-full rounded-lg text-left outline-none focus-visible:ring-2 focus-visible:ring-[#1B4FD8]/30"
      ...
      >
        ...
        {showEstablishment && establishmentName ? (
          <span className="mt-1 inline-flex rounded-full bg-white/80 px-2 py-0.5 text-[10px] font-semibold text-[#7D7B75]">
            {establishmentName}
          </span>
        ) : null}
...
export function SignalFeedDesktopRow({
  ...
  return (
    <article className="flex flex-col gap-1.5 rounded-xl border border-[#E8E6DF] bg-white px-3 py-2">
      ...
      <button
        type="button"
        className="flex w-full flex-col gap-1.5 rounded-lg px-1 py-0.5 text-left outline-none focus-visible:ring-2 focus-visible:ring-[#1B4FD8]/30"
        onClick={() => onSelect(item.id)}
      >
        ...
        <span className="flex w-full min-w-0 items-center justify-between gap-2">
          <span className={cn(lineClassName, 'min-w-0 flex-1')}>
            {reporterName ? (
          ...
          {item.aggregation_count > 0 ? (
            ...
            >
              {formatSignalFeedAggregationBadge(item.aggregation_count)}
            </span>
          ) : null}
        </span>
      </button>
