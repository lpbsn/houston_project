...
def test_email_change_over_limit_returns_429_including_wrong_password(api_client, active_user):
    create_membership(user=active_user)
    csrf_token = ensure_csrf(api_client)
    ip_headers = _client_ip_headers("203.0.113.40")
    login_response = login(
        api_client,
        csrf_token,
        identifier=active_user.email,
    ...
    for _ in range(2):
        response = api_client.post(
            ...
            {"password": "wrong-password", "new_email": "next@example.com"},
            format="json",
            **auth,
            **ip_headers,
        )
    ...
    response = api_client.post(
        "/api/v1/auth/email-change/",
        {"password": "secret", "new_email": "next@example.com"},
        format="json",
        **auth,
        **ip_headers,
    )
...
def test_email_change_confirm_over_limit_returns_429_for_invalid_token(api_client):
    ip_headers = _client_ip_headers("203.0.113.41")
