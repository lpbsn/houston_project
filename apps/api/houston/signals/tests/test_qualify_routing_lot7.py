...
def test_collision_merges_without_third_signal_or_duplicate_links(api_client):
    ...
    assert not SignalSourceObservation.objects.filter(
        signal_id=source_id,
        observation=observation,
        link_type=SignalSourceObservation.LinkType.CREATED_FROM,
    ).exists()
    ...
    retry = api_client.post(
        _qualify_url(membership.establishment_id, source_id),
        data=_full_patch(taxonomy, issue_focus=focus),
        format="json",
        **auth_headers(token),
    )
...
def test_qualify_deleted_merged_source_returns_404(api_client):
    membership = build_api_membership(role=EstablishmentMembership.Role.OWNER)
    taxonomy = create_restaurant_v3_taxonomy(membership.establishment)
    ...
    response = api_client.post(
        _qualify_url(membership.establishment_id, missing_id),
        data=_full_patch(taxonomy),
        format="json",
        **auth_headers(token),
    )
