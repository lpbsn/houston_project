...
def build_action_plan_execution_feed_page(
    *,
    ...
    if cursor is not None:
        queryset = apply_action_plan_execution_feed_cursor(queryset, cursor)
    sorted_queryset = apply_action_plan_execution_feed_sorting(queryset)

    candidates = list(sorted_queryset[: page_size + 1])
